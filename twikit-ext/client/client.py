...
class Client(OriginalClient):
    ...
    async def remove_follower(self, user_id: str) -> bool:
        """Remove follower by blocking and immediately unblocking them"""
        try:
            # Block the user (this will remove them from followers)
    ...
    async def update_profile_photo(self, image_path: str) -> bool:
        """Update profile photo from image file path"""
        try:
            import base64
            ...
            data, response = await self.post(
                Endpoint.UPDATE_PROFILE_IMAGE, 
                data=payload, 
                headers=self._base_headers
            )
            return "profile_image_url" in data or "profile_image_url_https" in data
        except Exception:
            return False

    async def update_profile_banner(self, image_path: str) -> bool:
        """Update profile banner from image file path"""
        try:
            import base64
            with open(image_path, 'rb') as image_file:
                image_data = base64.b64encode(image_file.read()).decode()
            ...
            return "profile_banner_url" in data or "profile_banner_url_https" in data
        except Exception:
            return False
    ...
    async def __aexit__(self, *args) -> None:
        await self.close()
# Removed security session methods as requested.